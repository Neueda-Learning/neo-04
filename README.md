...
that role can only touch this repo's own `neobank-<env>-neo-04` stack. You never hold AWS
credentials yourself — everything that reaches AWS goes through a workflow in this repo.

The front-end image is built with `APP_BASE_PATH=/neo-04`, because in the deployed stack
...
|---|---|---|---|
| **dev** | [`/neo-04/`](http://neobank-dev-571740187.ap-southeast-1.elb.amazonaws.com/neo-04/) | [`/neo-04/health`](http://neobank-dev-571740187.ap-southeast-1.elb.amazonaws.com/neo-04/health) · [`/neo-04/info`](http://neobank-dev-571740187.ap-southeast-1.elb.amazonaws.com/neo-04/info) | [orchestrator](http://neobank-dev-571740187.ap-southeast-1.elb.amazonaws.com/) |
| **prod** | [`/neo-04/`](http://neobank-prod-294820685.ap-southeast-1.elb.amazonaws.com/neo-04/) | [`/neo-04/health`](http://neobank-prod-294820685.ap-southeast-1.elb.amazonaws.com/neo-04/health) · [`/neo-04/info`](http://neobank-prod-294820685.ap-southeast-1.elb.amazonaws.com/neo-04/info) | [orchestrator](http://neobank-prod-294820685.ap-southeast-1.elb.amazonaws.com/) |
...
A `reset` destroys **only this service's schema** (`neo_04`). Every other service and the
...
DB_URL=jdbc:mysql://localhost:3307/neo_04 ./mvnw spring-boot:run
```
...
```json
{ "serviceId": "neo04", "status": "ACCEPTED",
...
| Env | Default | What |
|---|---|---|
| `SERVICE_ID` | `neo04` | the id sent on callbacks — note **no `-a`**, unlike the repo name |
| `SERVICE_NAME` | `Fraud & AML Screening` | display name |
...
- `backend/` and `frontend/` **stay at the repo root** — the system compose builds
  `./neo-04/backend` and `./neo-04/frontend` by path.
