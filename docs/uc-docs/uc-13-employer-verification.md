# Module 4 · Fraud & AML Screening — UC 13 · Employer verification (CANDIDATE)
...
- Use case: 13 · Employer verification · track B · prerequisite: after 01–08 — STRETCH, needs a new mock · build shape: mock service + engine branch · primary screen: Alert Detail (employer card)
- Data effect: new mock + engine branch
- Platform rules (non-negotiable): the orchestrator is the only caller; the whole application arrives in the envelope (plus the v5 `outputs` block, Option A); steps are independent and re-orderable; the payload is NEVER stored — only `applicationId`; every module ships `GET /cases/{id}/applicant` proxying the orchestrator; big lists are empty by default and capped at 10 rows (≤10 hydration calls); ALL APIs are idempotent — same request twice, same result once; every endpoint appears in the service's OpenAPI 3.0 (Swagger) spec.

## Story

"Trellis Health Ltd" — does that company exist at all? Fake employers prop up fake incomes across fraudulent applications, and real banks check names against Companies House. There is no such registry in this project, which is exactly why this candidate is marked STRETCH: unlike every other screening candidate, it cannot be built from config and own records — the team must build a mock registry service too, the way modules 3, 6, 7 and 8 do.

## What it adds

- A NEW mock: a companies-registry REST service (own tiny app or WireMock) seeded with a company list, with dials for latency and a kill switch — this module's first and only external call, and a new row in the mocked register.
- The engine gains an employer check for employed applicants: employerName not found → REVIEW; registry unavailable → NO flag (screening must not park half the bank because a lookup is down — the deliberate opposite of module 2's registry-down choice, and worth defending in the demo).
- New reason code `SCR_EMPLOYER_UNVERIFIED` — a v5 contract addition, instructor applies it.
- matchResults gains an employer card: name queried, found/not-found/skipped, registry latency.

## Acceptance criteria

1. An employed applicant whose employerName is not in the mock registry → REVIEW with SCR_EMPLOYER_UNVERIFIED; found → no flag.
2. UNEMPLOYED, RETIRED and STUDENT applicants skip the check entirely — no registry call is made.
3. Registry unreachable → the check records skipped-unavailable, the outcome is UNCHANGED, and the case does NOT park — availability of a lookup never becomes an outcome.
4. The mock's kill switch and latency dial demonstrably work, and the mock appears in the what-we-mocked register with its misbehaviour documented.
5. All eight UC 01–08 ACs pass unchanged with the registry absent from the compose file.
...
Take it last, if at all: it is the only screening candidate that buys the integration-module experience — a real dependency, a real failure mode, a real availability decision — at the price of building and operating a second service. Ambitious teams get modules 3–8's hardest lesson; everyone else should stop at UC 12.
